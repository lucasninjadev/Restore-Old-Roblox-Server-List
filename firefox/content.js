...
const get = async (url) => {
  try {
    const request = await fetch(`https://${url}`, {credentials: 'omit'});
...
async function updateUser() {
  try {
    let userID = 1;
    await browser.storage.sync.get('userID').then(value => {
      userID = value.userID;
    });
    if (userID == undefined)
      userID = 1;
...
async function reloadServers(place) {
  let asc = false;
  await browser.storage.sync.get('descending').then(value => {
  ...
  maxPages = 1;
  await browser.storage.sync.get('listSize').then(value => {
