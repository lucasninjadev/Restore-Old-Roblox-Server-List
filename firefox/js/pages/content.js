...
(async() => {
  ...
  div.id = 'rorsl-panel';
  div.innerHTML = await fetch(browser.runtime.getURL('html/main.html')).then(res => res.text())
  ...
  async function updateUser() {
    try {
      let userName = "Roblox";
      await browser.storage.local.get('username').then(value => {
  ...
  async function preLoadServers() {
    ...
    await browser.storage.local.get('descending').then(value => {
      if (value.descending) targetServerIds.reverse();
    });
    
    await browser.storage.local.get('listSize').then(value => {
  ...
  document.getElementById('rorsl-advanced-settings').addEventListener('click', async event => {
    ...
    browser.runtime.sendMessage('rorsl open advanced settings');
  })
